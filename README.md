...
- **Real:** every place name, address, phone, hours and rating in `src/data/guides.js` came from staysantarosa.com's own restaurant/wine/attractions guide pages (Dining, Wine & Beer, Attractions) and its "Major Events" annual calendar (Events) — see the comment at the top of that file for exactly what's sourced vs. estimated, including the two hotels/Courthouse Square and the event-venue coordinates. The Dining guide now carries all 35 real restaurants staysantarosa.com lists across its restaurant-guide.html data and its /dining page (30 came with full structured data; 5 — Beer Baron Bar & Kitchen, Darbar Kitchen, Grossman's Noshery & Bar, The Goose & Fern, Jackson's Bar and Oven — only appear on /dining, so their address/website came from public listings and their coordinates are an estimate, flagged in the file's header comment.
