...
// Attribute names are kebab-case (stroke-width, …) because Preact sets
// unknown SVG props as raw attributes — no camelCase conversion.
...
export function ChevronLeft({ size = 20, ...rest }) {
  return html`<svg ...${base(size)} ...${rest}><path d="M15 18l-6-6 6-6" /></svg>`;
}

export function ArrowRight({ size = 18, ...rest }) {
  return html`<svg ...${base(size)} ...${rest}><path d="M5 12h14M13 6l6 6-6 6" /></svg>`;
}
...
export function XIcon({ size = 18, ...rest }) {
  return html`<svg ...${base(size)} ...${rest}><path d="M18 6L6 18M6 6l12 12" /></svg>`;
}
...
export function PhoneIcon({ size = 18, ...rest }) {
  return html`
    <svg ...${base(size)} ...${rest}>
      <path
        d="M4.5 4h4l2 5-2.5 1.5a11 11 0 0 0 5.5 5.5L15 13l5 2v4a2 2 0 0 1-2 2A16 16 0 0 1 2.5 6a2 2 0 0 1 2-2z"
      />
    </svg>
  `;
}
...
export function PawIcon({ size = 14, ...rest }) {
  return html`
    <svg ...${base(size)} fill="currentColor" stroke="none" ...${rest}>
      ...
      <path
        d="M14 11.2c3.6 0 6.5 2.6 6.5 5.8 0 2-1.6 3.6-3.6 3.6-1.2 0-1.9-.5-2.9-.5s-1.7.5-2.9.5c-2 0-3.6-1.6-3.6-3.6 0-3.2 2.9-5.8 6.5-5.8z"
      />
    </svg>
  `;
}
...
export function WifiOffIcon({ size = 18, ...rest }) {
  return html`
    <svg ...${base(size)} ...${rest}>
      <path
        d="M2 8.5a16 16 0 0 1 5-3.2M22 8.5a16 16 0 0 0-8.4-4.4M6.5 12.5a9.6 9.6 0 0 1 4-2.1M17.5 12.5a9.6 9.6 0 0 0-2.6-1.7"
      />
...
// hand with the raw-SVG versions in components/MapView.js, which builds
// Leaflet divIcon markup as plain HTML strings rather than components.
...
export function GuideIcon({ guideKey, size = 18, ...rest }) {
  ...
  return Icon ? html`<${Icon} size=${size} ...${rest} />` : null;
}
