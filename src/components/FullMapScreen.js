...
// browsing here — just the stops you've added, each as a labeled card,
// with a route line between them and a Next stop banner. A day filter
// (All / Day 1 / Day 2 / ...) narrows the route down to one day at a time
// for a multi-day trip; "All" keeps every stop, colored by day.
export default function FullMapScreen({
  ...
  if (nextStop) {
    ...
    nextStopDistance =
      miles <= 1
        ? `${walkMinutes(miles)} min walk${fromLabel}`
        : `${driveMinutes(miles)} min drive${fromLabel}`;
  }
      ...
      <div className="full-map-canvas-wrap">
        ...
        html`
          <button
            ...
            onClick=${() => onOpenPlace(nextStop, nextStop.guideKey)}
          >
            <span className="next-stop-kicker">Next stop</span>
            <span className="next-stop-name">${nextStop.name}</span>
            ${nextStopDistance && html`<span className="next-stop-distance">${nextStopDistance}</span>`}
          </button>
        `}
