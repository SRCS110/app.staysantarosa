...
import WeatherNudge from './WeatherNudge.js';
import ItineraryPicks from './ItineraryPicks.js';
import FeaturedPicks from './FeaturedPicks.js';
import StubList from './StubList.js';
import { BellIcon, BellOffIcon, CalendarIcon, ArrowRight, GearIcon } from './icons.js';
        ...
        <span className="screen-topbar-actions">
          ...
          html`
            <button
              ...
              aria-label=${notifyOn
                ? 'Turn off closing-soon reminders'
                : 'Turn on closing-soon reminders'}
              title=${notifyOn ? 'Closing-soon reminders on' : 'Get closing-soon reminders'}
            >
              ${notifyOn ? html`<${BellIcon} size=${16} />` : html`<${BellOffIcon} size=${16} />`}
            </button>
          `}
      ...
      <div className="screen-body">
        ...
        html`
          <div className="shared-plan-banner">
            <span>
              ${`Someone shared a plan with you — ${sharedPlan.length} ${
                sharedPlan.length === 1 ? 'stop' : 'stops'
              }.`}
            </span>
            <div className="shared-plan-actions">
              <button type="button" className="btn btn-ghost" onClick=${onDismissShared}>Dismiss</button>
        ...
        ${installEvent &&
        !installDismissed &&
        html`<${InstallPrompt} onInstall=${onInstall} onDismiss=${onDismissInstall} />`}
        ...
        <button type="button" className="events-teaser" onClick=${() => onSelectTab('events')}>
          ...
          <span className="events-teaser-body">
            ...
            <span className="events-teaser-sub"
              >Festivals, releases and music around Sonoma County</span
            >
          </span>
          <${ArrowRight} size=${16} />
        </button>
