...
import { ChevronLeft, BedIcon, PencilIcon } from './icons.js';
...
export default function TripScreen({
  ...
  const hotelOptions = [
    { key: 'artHouse', label: hotels.artHouse.fullName, hint: 'A few blocks from Old Courthouse Square' },
    {
      ...
      label: hotels.courthouseSquare.fullName,
      hint: 'Right on the square — the app default',
    },
  ];
  ...
  return html`
    <div className="screen screen-trip">
      <div className="screen-topbar">
        ...
        <span className="screen-pill screen-pill-static"
          >${planCount ? `${planCount} in plan` : 'No stops yet'}</span
        >
      </div>
      ...
      <div className="screen-body trip-body">
        ...
        <p className="trip-page-sub">
          Free to use, no account or sign-in. These settings stay on this device and personalize
          every walking and driving time in the app.
        </p>
            ...
            ${hotelOptions.map(
              (opt) => html`
                <button
                  key=${opt.key}
                  type="button"
                  className=${`trip-hotel-option${
                    hotelKey === opt.key ? ' trip-hotel-option-active' : ''
                  }`}
        ...
        <button
          type="button"
          className="btn btn-primary btn-block trip-save-btn"
          onClick=${save}
        >
