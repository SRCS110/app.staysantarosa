...
import { CalendarIcon, GearIcon } from './icons.js';
    ...
    <div className="screen screen-events">
      ...
      <div className="screen-body events-body">
        ...
        <div className="events-list">
          ${events.length === 0 &&
          html`<p className="stub-empty">No upcoming events on the calendar right now.</p>`}
          ${events.map((e) => {
            ...
            return html`
              <div key=${e.name} className="event-row">
                <button
                  type="button"
                  className="event-row-tap"
                  onClick=${() => onOpenPlace(e, 'events')}
                >
                  ...
                  <span className="event-note">
                    ${e.note}${e.walk
                      ? ` · ${e.walk.min} min${e.walk.mode === 'drive' ? ' drive' : ' walk'}`
                      : ''}
                  </span>
                </button>
                <button
