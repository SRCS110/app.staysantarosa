...
import { CloudRainIcon, XIcon } from './icons.jsx';
  ...
  return (
    <div className="weather-nudge">
      <div className="weather-nudge-head">
        ...
        <button type="button" className="weather-nudge-dismiss" onClick={onDismiss} aria-label="Dismiss">
          <XIcon size={13} />
        </button>
      </div>
      <div className="weather-nudge-row">
        {indoorPlaces.map((p) => (
          <button key={p.name} type="button" className="weather-nudge-chip" onClick={() => onOpenPlace(p, 'attractions')}>
            {p.name}
          </button>
        ))}
      </div>
