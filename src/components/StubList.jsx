...
import { GuideIcon, StarIcon } from './icons.jsx';
...
export default function StubList({ guide, onOpenPlace, isInPlan, onTogglePlan, homeLabel = 'Courthouse Square' }) {
  ...
  const availableTags = useMemo(() => {
    ...
    const seen = new Set();
    guide.places.forEach((p) => p.tags.forEach((t) => {
    ...
    <div className="stub-sheet">
      ...
      <div className="stub-header">
        <div>
          ...
          <p className="stub-count">
            ...
            {activeTags.length ? ` of ${guide.places.length}` : ''}
          </p>
        </div>
      </div>
      ...
      <div className="stub-list">
        {visiblePlaces.length === 0 && (
          <p className="stub-empty">Nothing matches those filters — try clearing one.</p>
        )}
        {visiblePlaces.map((p) => {
              ...
              <button type="button" className="stub-row-tap" onClick={() => onOpenPlace(p)}>
                ...
                <span className="stub-info">
                  <span className="stub-name-row">
                    ...
                    {status && (
                      <span className={`stub-open-dot${status.open ? ' stub-open-dot-open' : ''}`} aria-hidden="true" />
                    )}
                  </span>
                  <span className="stub-note">
                    {p.note}
                    {p.rating != null && (
                      <span className="stub-rating">
                        <StarIcon size={10} /> {p.rating.toFixed(1)}
                      </span>
                    )}
                  </span>
                </span>
